...
public class Configuration {
	...
	// 默认中文停止词典文件名
	private static final String CN_STOP = "/cn_stopword.dic;";
	...
	public static List<String> getExtDictionarys() {
		...
		String extDictCfg = null;
		try {
			extDictCfg = EvaluateUtils
					.evaluateString(
							ConfigItemManifest.COM_MARSWORK_FULLTEXT_EXTDICT
									.getValue(), CFG.props
									.getProperty(EXT_DICT));
...
//		String extStopWordDictCfg = CFG.props.getProperty(EXT_STOP);
		String extStopWordDictCfg = CN_STOP;
		try {
			extStopWordDictCfg += EvaluateUtils
					.evaluateString(
							ConfigItemManifest.COM_MARSWORK_FULLTEXT_EXTSTOPWORD
									.getValue(), CFG.props
									.getProperty(EXT_STOP));
		} catch (PropertiesNotFoundException e) {
		}
